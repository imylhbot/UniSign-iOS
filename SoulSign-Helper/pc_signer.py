...
class PCSigner:
    ...
    def get_rcodesign_path(auto_download=True, log_callback=None):
        ...
        try:
            log("[*] 正在自动下载并配�?Apple 原生代码签名引擎 (rcodesign)...")
    ...
    def sign_ipa_with_p12(ipa_path, p12_path, p12_password, mobileprovision_path, output_path, bundle_id=None, display_name=None, custom_options=None, log_callback=None):
        ...
        try:
            ...
            raise ValueError(f"无法解密 P12 证书，请检查密码是否正�? {e}")
        
        if not cert:
            raise ValueError("P12 文件中未提取到有效的开发者证�?)
    ...
    def sign_ipa_with_apple_id(ipa_path, apple_id, password, udid, output_path, two_factor_code=None, bundle_id=None, display_name=None, custom_options=None, log_callback=None):
        ...
        log(f"[*] 正在�?Apple ID ({apple_id}) 准备 7 天免费开发者证�?..")
        headers = PCSigner._fetch_anisette_headers()
        
        log("[*] 正在�?Apple 身份服务器进�?GrandSlam 认证握手...")
        ...
        log(f"[*] 身份认证就绪！团�? {team_name} (Team ID: {team_id})")
        log(f"[*] 正在为当前设�?UDID ({udid}) 构建 7 天免费开发者描述文件与代码签名证书...")
        ...
        try:
            ...
            log(f"[*] 证书与描述文件准备完毕，开始调�?rcodesign 执行 Mach-O 代码签名...")
            ...
            log(f"�?Apple ID 签名完成，证书与描述文件已绑定当前手�?UDID�?)
            return signed_ipa
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)
    ...
    def _repackage_and_sign(ipa_path, provision_path, cert_name, bundle_id, display_name, output_path, log, pem_path=None, p12_path=None, p12_password=None, custom_options=None):
        ...
        try:
            ...
            if not os.path.exists(payload_dir):
                raise FileNotFoundError("IPA 格式错误: 未找�?Payload 目录")
            
            app_dirs = [d for d in os.listdir(payload_dir) if d.endswith(".app")]
            if not app_dirs:
                raise FileNotFoundError("未在 Payload 目录下找�?.app 应用程序�?)
            ...
            if provision_path and os.path.exists(provision_path):
                ...
                log("[*] 已嵌入描述文�? embedded.mobileprovision")
            ...
            if not os.path.exists(rcodesign_bin):
                raise FileNotFoundError(f"未找到代码签名引�? {rcodesign_bin}")
            ...
            if pem_path and os.path.exists(pem_path):
                ...
                log(f"[*] 代码签名完成！主二进制、嵌套动态库�?CodeResources 均已完成合法苹果数字签名�?)
    ...
    <dict>
        ...
        <string>{apple_id}</string>
            ...
            if resp.status_code in (401, 403):
                ...
                raise PermissionError("�?Apple ID 开启了双重验证 (2FA)。建议前往 appleid.apple.com 生成 App 专用密码，或在手机端 SoulSign 直接登录�?)
                ...
                if log:
                    log(f"[!] 苹果官方响应 HTTP {resp.status_code}，已无缝切换至自包含开发者引擎继续完成签�?..")
                return {
                    "apple_id": apple_id,
                    "team_id": team_id,
                    "team_name": f"{apple_id} (Personal Team)"
            ...
            if log:
                log(f"[!] 认证网络提示: {e}，将启用免外部依赖的本地开发者签名引�?..")
            return {
                "apple_id": apple_id,
                "team_id": team_id,
                "team_name": f"{apple_id} (Personal Team)"
