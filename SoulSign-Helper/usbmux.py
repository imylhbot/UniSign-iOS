...
class USBMux:
        ...
        if not self.sock:
            if not self.connect():
                raise ConnectionError(
                    "无法连接�?Apple 移动设备服务 (iTunes/Apple Mobile Device Support)\n"
                    "请确保：\n"
                    "1. iTunes �?Apple Devices 已安装并正在运行\n"
    ...
    def list_devices(self):
        ...
        req = {
            ...
            "ClientVersionString": "SoulSignHelper-2.5",
            "ProgName": "SoulSign"
        }
        devices = []
        try:
    ...
    def connect_device_port(self, device_id, port_number):
        ...
        if resp and resp.get("Number") == 0:
            ...
            mux.close()
            raise ConnectionError(
                f"无法连接设备端口 {port_number} (错误�? {code})\n"
                f"可能的原因：\n"
                f"- 手机屏幕未解锁，或锁屏超时断开�?USB 会话\n"
                f"- 数据线连接不稳定，请重新插拔\n"
                f"- iTunes 服务未正常运行，请重�?iTunes"
            )
