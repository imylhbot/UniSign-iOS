...
class SignViewController: UIViewController, UIDocumentPickerDelegate {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handleFileImported(_:)),
            name: NSNotification.Name("SoulSignFileImportedNotification"),
            object: nil
        )
    }
    ...
    private func setupUI() {
        ...
        // 2. Apple ID Selection Card
        setupAccountCard()
        contentStack.addArrangedSubview(accountCard)

        // 3. Customization Button
        customizeButton.setTitle("🛠️ 深度定制 (修改 Bundle ID / 名称 / 权限)", for: .normal)
    ...
    @objc private func handleFileImported(_ notification: Notification) {
